...
def vel_sweep():
    ...
    fig, ax = plt.subplots(1, 2, figsize=(2*3.4, 1.9), constrained_layout=True)
    ...
    plt.subplot(121)
    for i in range(len(vels)):
        plt.plot(mdmdts[i], mzs[i]/1000,label=r"$v_0=%1.2f$ km/s"%(vels[i]/1e3))
        plt.axhline(peak_alts[i],color="black")
    plt.ylim([70,110])
    plt.legend()
    plt.xlabel("dm/dt (kg/s)")
    plt.ylabel("Altitude (km)")
    plt.title("(Iron, 70 km/s, 45°, 100 $\mu g$)")
    plt.subplot(122)
    for i in range(len(vels)):
        plt.plot(mvels[i]/1e3, mzs[i]/1000,label=r"$v_0=%1.2f$ km/s"%(vels[i]/1e3))
        plt.axhline(peak_alts[i],color="black")
    plt.ylim([70,110])
...
#    plt.legend()
    ...
    plt.ylabel("Altitude (km)")
#    plt.title("Meteor Ablation Rate (Iron, 70 km/s, 45°)")

    #plt.gca().invert_yaxis()
    plt.show()
vel_sweep()

def rho_vel_sweep(rc=1.2):
    peak_alt_funs=[]
    rho_scales=[0.7,0.8,0.9,1.0,1.1,1.2,1.3]
    vels = np.linspace(30e3,72e3,num=10)
    all_rhos=[]
    all_vels=[]
    all_peak_alts=[]
    for r in rho_scales:
...
#            all_rhos.append(r)
 #           all_vels.append(v/1e3)
    ...
    for i in range(len(rho_scales)):
        plt.plot(vels/1e3,peak_alt_funs[i](vels/1e3),color="black",alpha=0.5,zorder=-1)

    #(0.9-1)/1
    for vi in range(len(vels)):
        
        v0=vels[vi]/1e3
        peak_alt=all_peak_alts[3][vi]
        v1 = (1/(rc**(1/3)))*v0
#        v1 = ( v0**3-(-dr)*v0**3)**(1/3) 
        plt.arrow(v0,peak_alt,v1-v0,0,color="red")



    plt.xlabel("Velocity (km/s)")
    plt.ylabel("Peak ablation height (km)")
    plt.show()

#   v=v0-(v0**3*(rho-rho0)/rho0)**(1/3) 
#   
#    (v0**3-v**3)/(v0**3) = (rho-rho0)/rho0
#    v0**3-(v0**3)*(rho-rho0)/rho0  = v**3 

# (v**3 - v0**3)/v0**3 = (rho-rho0)/rho0
# (v**3) (rho_a) = (((1/c**(1/3))*v)**3) (c*rho_a) 
# 
rho_vel_sweep()